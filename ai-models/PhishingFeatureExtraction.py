...
class FeatureExtraction:
    ...
    # Feature: Robots (Checks for presence and accessibility of robots.txt)
    def Robots(self):
        """
        Checks if a robots.txt file exists and is accessible for the domain.
        Returns:
            1: If robots.txt exists and is accessible (HTTP 2xx).
            0: If robots.txt is explicitly not found (HTTP 404).
            np.nan: If an error occurs during the request or for other status codes.
        """
        if not self.domain: return np.nan # Cannot check without a domain

        robots_url = f"{self.urlparse.scheme}://{self.domain}/robots.txt" # Use parsed scheme and domain

        try:
            # Use a reasonable timeout for this specific check
            # Use headers to appear more like a browser
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
            # Do not allow redirects for robots.txt check - it should be at the root
            response = requests.get(robots_url, timeout=5, headers=headers, verify=False, allow_redirects=False)

            if response.status_code >= 200 and response.status_code < 300:
                return 1 # Found and accessible
            elif response.status_code == 404:
                return 0 # Explicitly not found
            else:
                # Other status codes (e.g., 403 Forbidden, 5xx Server Error) are treated as uncertain/error
                # print(f"Warning: robots.txt check for {robots_url} returned status {response.status_code}") # Optional warning
                return np.nan
        except requests.exceptions.Timeout:
            # print(f"Warning: Timeout checking for robots.txt at {robots_url}") # Optional warning
            return np.nan # Treat timeout as unable to determine
        except requests.exceptions.RequestException as e:
            # print(f"Warning: Error checking for robots.txt at {robots_url}: {e}") # Optional warning
            return np.nan # Treat other request errors as unable to determine
        except Exception as e:
            # print(f"Warning: Unexpected error during robots.txt check for {robots_url}: {e}") # Optional warning
            return np.nan
        

    # Feature: NoOfPopup (Counts potential popup triggers in script tags)
    def NoOfPopup(self):
        """
        Counts occurrences of common JavaScript functions potentially used for pop-ups
        (like window.open) within <script> tags in the page source.
        This is a heuristic and may not be perfectly accurate.

        Returns:
            int: Count of potential triggers found.
            np.nan: If page source/soup is unavailable.
        """
        if self.soup is None: return np.nan # Need parsed HTML

        count = 0
        try:
            # Regex to find common popup-related functions (case-insensitive)
            # Looking for window.open, alert, confirm, prompt as potential indicators
            # This is basic and can be easily bypassed by obfuscation
            popup_pattern = re.compile(r'window\.open\(|alert\(|confirm\(|prompt\(', re.IGNORECASE)

            # Search within all <script> tag contents
            for script_tag in self.soup.find_all('script'):
                script_content = script_tag.string
                if script_content: # Check only scripts with inline content
                    matches = popup_pattern.findall(script_content)
                    count += len(matches)
            return count
        except Exception as e:
            # print(f"Warning: Error calculating NoOfPopup for {self.url}: {e}") # Optional warning
            return np.nan # Return NaN on error during script analysis
