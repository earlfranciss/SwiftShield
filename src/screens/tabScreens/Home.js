...
export default function Home({ navigation }) {
  ...
  const checkPermissions = async () => {
    try {
      ...
      const receiveSmsStatus =
        permissions[PermissionsAndroid.PERMISSIONS.RECEIVE_SMS];
      const notificationStatus =
        permissions[PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS];

      const coreSmsGranted =
  ...
  const requestAllPermissions = async () => {
    ...
    try {
      ...
      console.log(
        `Core SMS Granted: ${coreSmsGranted}, Notification Granted: ${notificationPermissionGranted}`
      );
      ...
      if (!notificationPermissionGranted && Platform.Version >= 33) {
        console.log(
          "Notification permission denied. Background alerts may not show."
        );
      }
  ...
  const startSmsListening = () => {
    ...
    if (!smsPermissionGranted) {
      ...
      Alert.alert(
        "Permissions Error",
        "Cannot start listener, SMS permissions are missing."
      );
      return;
    }
  ...
  const stopSmsListening = () => {
    if (isSmsListening && smsSubscriptionRef.current) {
      ...
      try {
        ...
        Alert.alert(
          "Protection Disabled",
          "No longer monitoring incoming SMS."
        );
      } catch (error) {
    ...
    try {
      if (isSmsListening) {
        ...
        if (permissionsOk) {
          ...
          Alert.alert(
            "Permissions Required",
            "SMS read/receive permissions are needed to enable protection. Please grant them when prompted or check App Settings."
          );
        }
      }
  ...
  const handleUrlScan = async () => {
    ...
    const urlPattern =
      /^(https?:\/\/)?(?!localhost)(?!.*:\d{2,5})([\w.-]+\.[a-zA-Z]{2,})(\/[^\s#]*)?$/;
 ...
 /* const handleClearInput = () => {
  ...
  const sendSmsToBackendForProcessing = async (smsData) => {
    ...
    try {
      ...
      if (!textResponse.ok) {
        ...
        console.error(
          `SMS Text Classification Error (${textResponse.status}):`,
          errorText
        );
      ...
      console.error(
        "Error calling SMS text classification endpoint:",
        textError
      );
    }
  };
  ...
  const unifiedScan = async (payload) => {
    ...
    console.log(
      `Sending to ${endpoint} for scanning:`,
      JSON.stringify({ url: payload.url })
    );
    ...
    try {
      ...
      if (payload.url && !payload.sender) {
        // Only show alert for manual URL scans
